...
/**
 * An application that serves as a sample to show how the Documents List Service can be used to search your documents and upload files.
 * 
 * 
 * 
 * 
...
public class DocumentListDemo {
	 ...
	 */
	private static final String[] USAGE_MESSAGE = { "Usage: java DocumentListDemo.jar --username <user> --password <pass>",
			"Usage: java DocumentListDemo.jar --authSub <token>", "    [--host <host:port>]          Where is the feed (default = docs.google.com)",
			"    [--log]                       Enable logging of requests", "" };

	/**
	 * Welcome message, introducing the program.
	 */
	private final String[] WELCOME_MESSAGE = { "", "This is a demo of the document list feed!",
			"Using this interface, you can read and upload your documents.", "Type 'help' for a list of commands.", "" };

	/**
	 * Help on all available commands.
	 */
	private final String[] COMMAND_HELP_MESSAGE = { "Commands:", "    create <object_type> <name>               [[create an object]]",
			...
			"    asearch <search_option>                   [[advanced search]]", "    upload <file_path> <title>                [[uploads a object]]",
	...
	private final String[] COMMAND_HELP_CREATE = { "create <object_type> <name>", "    object_type: document, spreadsheet, folder.",
			"    name: the name for the new object" };
	private final String[] COMMAND_HELP_TRASH = { "trash <resource_id> [delete]", "    resource_id: the resource id of the object to be deleted",
			"    \"delete\": Specify to permanently delete the document instead of just trashing it." };
	private final String[] COMMAND_HELP_DOWNLOAD = { "download <resource_id> <file_path>", "    resource_id: the resource id of the file you wish to download",
			"    file_path: the path to the directory to save the file in" };
	private final String[] COMMAND_HELP_LIST = { "list [object_type]",
			"    object_type: all, starred, documents, spreadsheets, pdfs, presentations, folders.\n" + "        (defaults to 'all')",
			"list folder <folder_id>", "    folder_id: The id of the folder you want the contents list for." };
	private final String[] COMMAND_HELP_MOVE = { "move <resource_id> <folder_id>", "   resource_id: the resource id of the object to be moved",
			"    folder_id: the folder to move the document into" };
	private final String[] COMMAND_HELP_PERMS = { "perms list <resource_id>",
			"    resource_id: the resource id of the object you wish to add/list permissions for", "perms add <role> <scope> <email> <object_id>",
			"    role: \"reader\", \"writer\", \"owner\"", "    scope: \"user\", \"domain\"",
			"    email: The user's email address or domain name (\"user@gmail.com\", \"domain.com\")",
			"    resource_id: The resource id of the object to change permissions for.", "perms change <role> <scope> <email> <object_id>",
			"    role: \"reader\", \"writer\", \"owner\"", "    scope: \"user\", \"domain\"",
			"    email: The user's email address or domain name (\"user@gmail.com\", \"domain.com\")",
			"    object_id: The id of the object to change permissions for.", "perms remove <scope> <email> <object_id>",
			"    role: \"reader\", \"writer\", \"owner\"", "    scope: \"user\", \"domain\"",
			"    email: The user's email address or domain name (\"user@gmail.com\", \"domain.com\")",
			"    object_id: The id of the object to change permissions for." };

	private final String[] COMMAND_HELP_REMOVE = { "remove <object_id> <folder_id>", "    object_id: the id of the object to remove from the folder",
			"    folder_id: the id of the folder to remove the object from" };
	private final String[] COMMAND_HELP_SEARCH = { "search <search_text>", "    search_text: A string to be used for a full text query" };
	private final String[] COMMAND_HELP_ASEARCH = { "asearch [<query_param>=<value>] [<query_param2>=<value2>] ...",
			"    query_param: title, title-exact, opened-min, opened-max, owner, writer, reader, " + "showfolders, etc.",
			"    value: The value of the parameter" };
	private final String[] COMMAND_HELP_UPLOAD = { "upload <file_path> <title>", "    file_path: file to upload", "    title: A title to call the document" };
	private final String[] COMMAND_HELP_REVISIONS = { "revisions <resource_id>", "    resource_id: document resource id" };
	private final String[] COMMAND_HELP_HELP = { "help [command]", "    Weeeeeeeeeeeeee..." };
	private final String[] COMMAND_HELP_EXIT = { "exit", "    Exit the program." };
	 ...
	 */
	public DocumentListDemo(PrintStream outputStream, String appName, String host) throws DocumentListException {
		out = outputStream;
	...
	public void printRevisionEntry(RevisionEntry entry) {
		...
		output.append(" by " + entry.getModifyingUser().getName() + " - " + entry.getModifyingUser().getEmail() + "\n");
	 ...
	 * 
	 * @param args
	 *            arguments for the "create" command. args[0] = "create" args[1] = object_type ("folder", "document", "spreadsheet") args[2]
	 *            = title (what to name the document/folder)
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 ...
	 * 
	 * @param args
	 *            arguments for the "trash" command. args[0] = "trash" args[1] = resourceid (the resource id of the object to be trashed)
	 ...
	 * @throws DocumentListException
	 */
	private void executeTrash(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length == 3) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "download" command. args[0] = "download" args[1] = resourceId (the resource id of the object to be
	 *            downloaded) args[2] = filepath (the path and filename to save the object as)
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 * @throws MalformedURLException
	 ...
	 * @throws DocumentListException
	 */
	private void executeDownload(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length == 3) {
			String docType = documentList.getResourceIdPrefix(args[1]);
			if (docType.equals("spreadsheet")) {
				String format = documentList.getDownloadFormat(args[1], getTypeFromFilename(args[2]));
				...
				if (!requestedFormat.equals(fileExtension)) {
					...
					String[] formatWarning = { "Warning: " + mc.getMimeType().getMediaType() + " cannot be downloaded as a " + requestedFormat + ". Using ."
	 ...
	 * 
	 * @param args
	 *            arguments for the "list" command. args[0] = "list" args[1] = category ("all", "folders", "documents", "spreadsheets",
	 *            "pdfs", "presentations", "starred", "trashed") args[2] = folderId (required if args[1] is "folder")
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 * @throws MalformedURLException
	 ...
	 * @throws DocumentListException
	 */
	private void executeList(String[] args) throws IOException, ServiceException, DocumentListException {
	 ...
	 * 
	 * @param args
	 *            arguments for the "move" command. args[0] = "move" args[1] = resourceid (the resourceid of the object to move) args[2] =
	 *            folderResourceId (the resource id of the folder to move the object to)
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 * @throws MalformedURLException
	 ...
	 * @throws DocumentListException
	 */
	private void executeMove(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length == 3) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "perms" command. args[0] = "perms" args[1] = "list" args[2] = resourceId args[1] = "add", "change"
	 *            args[2] = role args[3] = scope args[4] = email args[5] = resourceId args[1] = "remove" args[2] = scope args[3] = email
	 *            args[4] = resourceId
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 ...
	 * @throws DocumentListException
	 */
	private void executePerms(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length < 3) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "remove" command. args[0] = "remove" args[1] = resourceId args[2] = folderReourceId
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 * @throws MalformedURLException
	 ...
	 * 
	 * @param args
	 *            arguments for the "search" command. args[0] = "search" args[1] = searchString
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 * @throws MalformedURLException
	 ...
	 * @throws DocumentListException
	 */
	private void executeSearch(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length == 2) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "asearch" command. args[0] = "asearch"
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 * @throws MalformedURLException
	 ...
	 * @throws DocumentListException
	 */
	private void executeAdvancedSearch(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length <= 1) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "upload" command. args[0] = "upload" args[1] = filepath (path and filename of the file to be uploaded)
	 ...
	 * @throws DocumentListException
	 */
	private void executeUpload(String[] args) throws IOException, ServiceException, DocumentListException, InterruptedException {
		if (args.length == 3) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "upload" command. args[0] = "revisions" args[1] = resourceId (the resource id of the object to fetch
	 *            revisions for)
	 * 
	 * @throws IOException
	 *             when an error occurs in communication with the Doclist service.
	 ...
	 * @throws DocumentListException
	 */
	private void executeRevisions(String[] args) throws IOException, ServiceException, DocumentListException {
		if (args.length == 2) {
	 ...
	 * 
	 * @param args
	 *            arguments for the "help" command. args[0] = "help" args[1] = command
	 */
	 ...
	 * 
	 * @param filename
	 *            the filename to extract the type of file from.
	 */
	 ...
	 * 
	 * @param command
	 *            the entire command entered by the user to be broken up into arguments.
	 */
	 ...
	 * @throws ServiceException
	 */
	private boolean executeCommand(BufferedReader reader) throws IOException, ServiceException, InterruptedException {
	 ...
	 * @throws IOException
	 */
	public static void main(String[] args) throws DocumentListException, IOException, ServiceException, InterruptedException {
		...
		DocumentListDemo demo = new DocumentListDemo(System.out, APPLICATION_NAME, host);
