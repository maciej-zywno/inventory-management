package com.ebayinventory.ebay;

import java.util.List;

import org.springframework.context.ApplicationContext;
import org.springframework.context.support.ClassPathXmlApplicationContext;

import com.ebay.sdk.ApiException;
import com.ebay.sdk.SdkException;
import com.ebay.soap.eBLBaseComponents.ItemType;
import com.ebayinventory.model.Token;

public class EbaySellerFacadeTest {

	public static void main(String[] args) throws ApiException, SdkException, Exception {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("spring/main.xml");
		EbaySellerFacade facade = ctx.getBean(EbaySellerFacade.class);
		Token token = new Token(
				"AgAAAA**AQAAAA**aAAAAA**9QZ2Tw**nY+sHZ2PrBmdj6wVnY+sEZ2PrA2dj6wFk4CoAJiAoQ6dj6x9nY+seQ**hWwBAA**AAMAAA**");
		List<ItemType> activeItemTypes = facade.getActiveItemTypes(token);
		System.out.println(activeItemTypes.size());
	}

}
